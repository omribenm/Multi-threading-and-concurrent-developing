...
public abstract class MicroService implements Runnable {
    ...
    private ConcurrentHashMap<Class< ? extends Message> , Callback<?>> callbacktoReturn;
    private ConcurrentHashMap<Request<?>, Callback<?>> waitComp;
    /**
    ...
    public MicroService(String name) {
    	...
    	callbacktoReturn=new ConcurrentHashMap<Class< ? extends Message> , Callback<?>>();
    	waitComp=new ConcurrentHashMap<Request<?>, Callback<?>> ();
        this.name = name;
    }
    ...
    protected  final <R extends Request> void subscribeRequest(Class<R> type, Callback<R> callback) {
    			...
    			callbacktoReturn.put(type,callback);	
       ...
       // messageBus.register(this);
        while (!terminated) {
        	try {
        			...
        			Message mes = messageBus.awaitMessage(this);
        			
        			
        			if(mes.getClass()==RequestCompleted.class){
        				
        			Callback d = waitComp.get(((RequestCompleted)mes).getCompletedRequest());
        			
        	
            		d.call(((RequestCompleted)mes).getResult());
            		
        		}
        			
        		else{
        			
        			Callback c = callbacktoReturn.get(mes.getClass());
            		c.call(mes);
